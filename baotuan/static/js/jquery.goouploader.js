...
function __GooUploader(Div,property){
    this.$multiple=false;
    ...
    this.$swfUpload.uploadSuccess=function(file,msg){
		...
		if (data.status == 'ok')
		{
			...
			li.find('.result').html(property.op_ok_text);
			if (typeof(property.uploaded_callback) === 'function')
			{
				property.uploaded_callback(data.file);
			}
		}
        else
		{
			li.children(".op_fail").css("display","block");
			li.find('.result').html(data.msg);
		}
    };
    this.$swfUpload.uploadError=function(file,code,msg){
        ...
        li.children(".op_no").css("display","none");
        li.children(".op_fail").css("display","block");
		li.find('.result').html(property.op_fail_text);
    };
...
jQuery.createGooUploader = __GooUploader;