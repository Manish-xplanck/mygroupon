...
function Execute(){
	...
	function Warnofticket(){
		...
		'title'=>'购物券即将到期提示信息',
		'content'=>'温馨提示您购买的产品（'.$ticket['name'].'）消费券即将于'.date('Y-m-d', $ticket['perioddate']).'到期请尽快消费以免过期，请点<a href="'.$this->Config['site_url'].'">这里</a>查看您的团购券！',
		'addtime'=>time()
		);
		$keys=$values='';
		foreach($ary as $i => $valuez){
...
function Doreplyquestion(){
	...
	function SiteLogoManager()
	{
		...
		$names = array('','','蓝色风格','喜庆风格','紫色风格','清爽风格');
		for ($i=2; $i<=5; $i++)
		{
			$logos[$i] = array(
				'title' => $names[$i],
